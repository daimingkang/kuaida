...
@section('jumbotron')
    ...
    <div class="jumbotron text-center">
        <h4>{{ Setting()->get('website_welcome','现在加入Tipask问答网，一起记录站长的世界') }} <a class="btn btn-primary ml-10" href="{{ route('auth.user.register') }}" role="button">立即注册</a> <a class="btn btn-default ml-5" href="{{ route('auth.user.login') }}" role="button">用户登录</a>&nbsp;&nbsp;<img width="30px" src="{{url('static/images/qq.png')}}">&nbsp;<a href="{{url('/oauth/qq/login')}}">QQ登录</a>&nbsp;&nbsp;<img width="30px" src="{{url('static/images/wxlogin1.png')}}">&nbsp;<a href="{{url('/oauth/weixin/login')}}">微信登录</a></h4>
    </div>
    @endif
@endsection
